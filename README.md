...
10. [Stage2_MayrParSho_Hackett_mean phylogeny_ultrametric.tre](Data/Raw/Stage2_MayrParSho_Hackett_mean phylogeny_ultrametric.tre): Bird phylogeny, ultrametric consensus tree based on 1,000 credible phylogenies available from VertLife.
...
Note: Range map data from the IUCN Red List and BirdLife International cannot be reposted or redistributed. We are working on getting permission to make publicly available the data products derived from the range map data.
...
4. XXXX_cell_df.csv: Lists of species in each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_cell_df.csv
    - mamm_cell_df.csv
    - bat_cell_df.csv
5. XXXX_SR_cells.csv: Species richness of each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_SR_cells.csv
    - mamm_SR_cells.csv
    - bat_SR_cells.csv
6. XXXX_PD_cells.csv: Phylogenetic diversity of each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_PD_cells.csv
    - mamm_PD_cells.csv
    - bat_PD_cells.csv
7. XXXX_FR_cells.csv: Functional richness of each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_FR_cells.csv
    - mamm_FR_cells.csv
    - bat_FR_cells.csv
8. XXXX_mean_fb.csv: Mean functional beta diversity turnover of each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_mean_fb.csv
    - mamm_mean_fb.csv
    - bat_mean_fb.csv
9. XXXX_pb_NMDS.csv: Phylobetadiversity turnover NMDS coordinates for 3 axes for each grid cell (not publicly available). Separate files for birds, mammals, and bats.
    - bird_pb_NMDS.csv
    - mamm_pb_NMDS.csv
    - bat_pb_NMDS.csv
10. XXXX_pb_dist.RDS: Distance matrix of phylobetadiversity turnover (not publicly available). Separate files for birds, mammals, and bats.
    - bird_pb_dist.RDS
    - mamm_pb_dist.RDS
    - bat_pb_dist.RDS
11. XXXX_fb.RDS: Output from functional beta diversity calculation, including distance matrices for total beta diversity, turnover, and nestedness (not publicly available). Separate files for mammals and bats.
    - mamm_fb.RDS
    - bat_fb.RDS
12. XXXX_fb_turn.txt: Distance matrix of functional beta diversity turnover (not publicly available). Separate files for birds, mammals, and bats.
    - bird_fb_turn.txt
    - mamm_fb_turn.txt
    - bat_fb_turn.txt
